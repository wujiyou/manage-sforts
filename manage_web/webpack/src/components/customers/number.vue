<template>
  <div>
    <div style="background:white;width:99.1%;padding-left:15px">
      <my-bread level1="综合查询" level2="液化气瓶" level3="气瓶数量状态"></my-bread>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getUserlist() {
      const res = await this.$http.get(
        `/public/gas/manageGasBasicCylinder/list?pageSize=${this.pageSize}&currPage=${this.currPage}&gasId=${this.gasId}`
      );
      // &name=${this.name}
      console.log(res);
      this.userlist = res.data.data.list;
