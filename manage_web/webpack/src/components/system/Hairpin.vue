<template>
  <div>
    <div style="background:white;width:99.1%;padding-left:15px">
      <my-bread level1="预警管理" level2="系统监管" level3="气瓶发卡日志"></my-bread>
    </div>
    <el-card class="box-card" style="margin:20px 20px 0 20px;">
      ...
      <el-col style="margin-bottom:5px;margin-top:5px" :span="24">
        ...
        <el-col :span="2">
          <el-button size="small" type="danger" style="font-size:13px" plain icon="el-icon-delete" @click="showDeleUserMsgBox">删除</el-button>
        </el-col>
      </el-col>
      <!-- 表格 -->
      <template>
...
export default {
  ...
  methods: {
    ...
    async searchUser() {
      ...
      if (dering == "") {
        ...
        const res = await this.$http.get(
          `/operation-logs/findPage?page=${this.page}&rows=${this.rows}&createdTime=${resDate}&username=${this.username}&type=2`
        );
        console.log(res);
    ...
    async getUserlist() {
      const res = await this.$http.get(
        `/operation-logs/findPage?page=${this.page}&rows=${this.rows}&username=${this.username}&createdTime=${this.createdTime}&type=2`
      );
      console.log(res);
      this.userlist = res.data.data.list;
      this.total = res.data.data.totalCount;
