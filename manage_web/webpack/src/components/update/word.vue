<template>
  <div class="body">
    <div class="title">液化气瓶大数据</div>
    
    <div class="box">
      <button class="backBtn" @click="backs()">返回上级</button>
  ...
  methods: {
    ...
    mapChart() {
      ...
      myMapChart.on("click", function(params) {
        if (provinces[params.data.name] !== undefined) {
          // 如果点击的是34个省、市、自治区，绘制选中地区的二级地图
          $.getJSON(
            "/static/map/province/" + provinces[params.data.name] + ".json",
            function(data) {
              console.log(data);
        ...
        } else if (provinces[params.seriesName] !== undefined) {
          ...
          console.log(cityDeta.features[da].geometry.coordinates[0][0][0][0]);
          console.log(cityDeta.features[da].geometry.coordinates[0][0][0][1]);
