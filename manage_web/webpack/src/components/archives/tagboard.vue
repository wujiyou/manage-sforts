  ...
  methods: {
    ...
    sendLabel(obj) {
      ...
      try {
        ...
        this.$http.post(`/send/getAppId?appId=${objs.cardno}&type=1`).then((res) => {
          console.log(res);
          if (res.data.data < 1) {
            this.$http
              .post(`/send/updateTuckComb?appid=${objs.cardno}&id=${obj}`)
              .then((val) => {
                console.log(val);
                console.log("--------------------");
        ...
        // cylinder-card/getAppId?appId=${objs.cardno}&type=1
      } finally {
    ...
    async searchUser() {
      const res = await this.$http.get(
        `/cylinder-card/findPage?page=${this.page}&rows=${this.rows}&regId=${this.regId}&isCard=${this.isCard}&gasId=${this.gasId}&buildingUser=${this.buildingUser}`
      );
      console.log(res);
