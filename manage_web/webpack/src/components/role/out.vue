<template>
  <div>
    <div style="background:white;width:99.1%;padding-left:15px">
      <my-bread level1="预警管理" level2="充装记录" level3="超期气瓶扫描记录"></my-bread>
    </div>
    <el-card class="box-card" style="margin:20px 20px 0 20px;">
      ...
      <template>
        ...
        >
          ...
          <el-table-column show-overflow-tooltip prop="unitName" label="加气站名称" width="200"></el-table-column>
          ...
          <el-table-column sortable show-overflow-tooltip prop="scanTime" label="扫描时间">
            <!-- <template slot-scope="socpe">{{socpe.row.scanTime | fmtdate}}</template> -->
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // scanning/recordCar/singleList?String unitName, String gasId, Date scanTime,Date endTime, Integer currPage, Integer pageSize
    async searchUser() {
      const res = await this.$http.get(
        `scanning/record/list?pageSize=${this.pageSize}&currPage=${this.currPage}&unitName=${this.unitName}&regId=${this.regId}&scanTime=${this.scanTime}&endTime=${this.endTime}`
      );
    ...
    async getUserlist() {
      // http://localhost:8081/public/gas/CylinderFilling/CylinderFillingPaging?pageSize=2&currPage=2&regId=01&regId=1588923670165&createTime=2020-05-10 10:44:22&createTime2=2020-05-11 10:44:22

      const res = await this.$http.get(
        `scanning/record/list?pageSize=${this.pageSize}&currPage=${this.currPage}&unitName=${this.unitName}&regId=${this.regId}&scanTime=${this.scanTime}&endTime=${this.endTime}`
      );
      // &name=${this.name} unitName
      console.log(res);
      this.userlist = res.data.data.list;
      this.total = res.data.data.totalCount;
    }
