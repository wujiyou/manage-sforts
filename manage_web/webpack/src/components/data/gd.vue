<template>
  <div class="container">
    ...
    <div class="appContainer" @click="hideMenu">
      <div class="big_head">
        <div class="big_head_title">工业气瓶大数据</div>
      ...
      <div class="map_box">
        <div>
          <!-- <div  style="background:white;width:99.1%;padding:10px 0px 15px 15px"><my-bread level1="数据平台" level2="工业气瓶" level3="气瓶地图显示"></my-bread></div> -->
        ...
        <!-- <div class="map_center_title">长沙工业气瓶分布图</div> -->
           ...
           :total="400"分页总数
            ...
            <el-dialog title="查看信息" :visible.sync="dialogTableVisible2">
              ...
              <el-table :data="gridData2" border>
                ...
                <el-table-column prop="appid" label="产品编号"></el-table-column>
            ...
            <el-dialog title="查看信息" :visible.sync="dialogTableVisible5">
              ...
              <el-table border :data="gridData11">
                ...
                <el-table-column show-overflow-tooltip prop="name" label="员工姓名"></el-table-column>
...
export default {
  ...
  async created() {
    ...
    this.change5();
    this.change6();
    const res6 = await this.$http.get(`/industry-stats/map`);
    console.log(res6);
    ...
    //   if (e.isStatus == "0") {
    //     e.isStatus = img0;
    //   } else if (e.isStatus == "1") {
    //     e.isStatus = img1;
    //   } else if (e.isStatus == "2") {
    //     e.isStatus = img2;
    //   } else if (e.isStatus == "3") {
    //     e.isStatus = img3;
    //   }
    // });
    this.markerPoint.forEach(e => {
      console.log(e);
      // e.isStatus == "0" ? (e.status = img0) : (e.status = img1);
      e.isStatus == "1" ? (e.status = img1) : (e.status = img0);
      // e.isStatus == "2" ? (e.status = img2) : (e.status = img3);
      // e.isStatus == "3" ? (e.status = img3) : (e.status = img);
    });
  },
  filters: {
  ...
  methods: {
    ...
    async change2() {
      //不合格数量
      const res9 = await this.$http.get(`/industry-stats/checkState/0`);
      this.bhege = res9.data.data;

      //即将超期数量
      const res4 = await this.$http.get(`/industry-stats/checkState/3`);
      this.hei = res4.data.data;

      //合格数量
      const res1 = await this.$http.get(`/industry-stats/checkState/1`);
      this.hege = res1.data.data;

      //检验状态
      // const res = await this.$http.get(`/industry-stats/checkState`);
      // this.markerPoint1 = res.data.data;
      // this.hege = res.data.data[0].count;
      // this.bhege = res.data.data[1].count;
      // this.chaoqi = res.data.data[2].count;
      // this.yuqi = res.data.data[3].count;

      // this.bhege = res.data.data;
    },
    async change3() {
      // 充装  配送 已登记气瓶数  检验站 发证员工
      //逾期未检数量
      const res10 = await this.$http.get(`/industry-stats/checkState/2`);
      this.yuqi = res10.data.data;

      //发证员工
      const res = await this.$http.get(`/industry-stats/issuCardNum`);
      console.log(res);
      this.fa = res.data.data;

      //检验状态合格表单
      const res11 = await this.$http.get(`/industry-stats/findByTypeAndUnit/1`);
      console.log(res11);
    ...
    async change4() {
      ...
      const res12 = await this.$http.get(
        `/industry-stats/findUnit/2?pageSize=${this.pageSize}&currPage=${this.currPage}`
      );
      console.log(res12);
      ...
      const res13 = await this.$http.get(
        `/industry-stats/findUnit/5?pageSize=${this.pageSize}&currPage=${this.currPage}`
      );
      ...
      const res18 = await this.$http.get(
        `/industry-stats/findUnit/3?pageSize=${this.pageSize}&currPage=${this.currPage}`
      );
      this.gridData8 = res18.data.data.list;
      this.total4 = res18.data.data.totalCount;

      const res19 = await this.$http.get(`stats/findDoucument`);
      this.gridData10 = res19.data.data;

      //发证员工详细信息
      const res11 = await this.$http.get(
        `/industry-stats/issuCard?pageSize=${this.pageSize}&currPage=${this.currPage}`
      );
      this.gridData11 = res11.data.data.list;
      this.total5 = res11.data.data.totalCount;

      //已登记气瓶数详细信息
      const res = await this.$http.get(
        `/industry-stats/findDoucument?pageSize=${this.pageSize}&currPage=${this.currPage}`
      );
      this.gridData2 = res.data.data.list;
      this.total2 = res.data.data.totalCount;
    },

    async change5() {
      //即将超期表单
      const res14 = await this.$http.get(`/industry-stats/findByTypeAndUnit/3`);

      this.gridData5 = res14.data.data.list;

      //不合格表单
      const res15 = await this.$http.get(`/industry-stats/findByTypeAndUnit/0`);
      this.gridData6 = res15.data.data.list;

      //超期未检表单
      const res16 = await this.$http.get(`/industry-stats/findByTypeAndUnit/2`);
      console.log(res16);
    ...
    async drawMap2() {
      ...
      const res = await this.$http.get(`/industry-stats/findByUseUnit`);
      console.log(res);
    ...
    async drawMap3() {
      //本月气站活跃情况
      const res17 = await this.$http.get(`/industry-stats/RecordNumber`);
      const datad1 = [];
      const datadd1 = [];
    ...
    async drawMap4() {
      //数量趋势
      const res = await this.$http.get(`/industry-stats/countYear`);
      console.log(res);
    ...
    async getUserlist() {
      const res = await this.$http.get(`/industry-stats/findUnitNumber/2`);
      this.cong = res.data.data;

      const res1 = await this.$http.get(`/industry-stats/findUnitNumber/5`);
      this.pei = res1.data.data;

      const res6 = await this.$http.get(`/industry-stats/findUnitNumber/3`);
      this.jianyan = res6.data.data;

      const res2 = await this.$http.get(`/industry-stats/findBottleNumber`);
