<template>
  <div class="container">
    ...
    <div class="appContainer" @click="hideMenu">
      ...
      <div class="map_box">
        <div>
          <!-- <div  style="background:white;width:99.1%;padding:10px 0px 15px 15px"><my-bread level1="数据平台" level2="车用气瓶" level3="气瓶地图显示"></my-bread></div> -->
          ...
          >
            ...
            >
              ...
              >
                <p>
                  <span>单位名称:</span>
                  <span v-text="item.unitName"></span>
                </p>
                <p>
                  <span>单位负责人:</span>
                  <span v-text="item.personnelName"></span>
                </p>
                <p>
                  <span>手机号:</span>
                ...
                <!-- <p>
                  <span>是否在线:</span>
                  <span v-text="item.isAlive">
                    <template slot-scope="scope">
                      <p v-if="userlist[scope.$index].isAlive==0">不在线</p>
                      <p v-if="userlist[scope.$index].isAlive==1"></p>
                    </template>
                  </span>
                </p>-->
                <!-- <h1>客户信息</h1>
            ...
            <div class="tishi">
              <div>
                <span style="color:#23cdfd">设备状态:</span>
                <div class="anchor">
                  <img :src="tishi.buzx" />
                  <span style="color:#d81e06;">不在线</span>
                </div>
                <div class="anchor">
                  <img :src="tishi.zx" />
                  <span style="color:#1afa29;">在线</span>
                </div>
              </div>
              <!-- <div class="anchor">
        ...
        <!-- <div class="map_center_title">长沙车用气瓶分布图</div> -->
        ...
        <div class="big_left_chart1_box">
          ...
          :current-page 设置当前页是第几页
           ...
           :total="400"分页总数
            ...
            <el-dialog title="查看信息" :visible.sync="dialogTableVisible2" style="width:100%;">
              <span slot="title">
                <span style="color: #23cdfd">查看信息</span>
              </span>
              <el-table :data="gridData2" border>
                ...
                <el-table-column prop="makeData" label="制造年月"></el-table-column>
                <el-table-column prop="pressure" label="公称工作压力(Mpa)"></el-table-column>
                <el-table-column prop="volume" label="容积(L)"></el-table-column>
                <el-table-column prop="checkData" label="最近一次检验日期"></el-table-column>
                <el-table-column prop="nextCheckData" label="下次检验日期"></el-table-column>
                <el-table-column prop="selfId" label="自身编号"></el-table-column>
                <el-table-column prop="useStatus" label="变更或者停用情况">
                  <template slot-scope="scope">
                    <p v-if="gridData2[scope.$index].checkStatus==0">新增</p>
                    <p v-if="gridData2[scope.$index].checkStatus==1">停用</p>
                    <p v-if="gridData2[scope.$index].checkStatus==2">注销</p>
                    <p v-if="gridData2[scope.$index].checkStatus==3">报废</p>
                  </template>
                </el-table-column>
              </el-table>
              <el-pagination
            ...
            <el-dialog title="查看信息" :visible.sync="dialogTableVisible8">
              ...
              <el-table border :data="gridData10">
                <el-table-column show-overflow-tooltip prop="name" label="单位名称"></el-table-column>
                <el-table-column show-overflow-tooltip prop="licence" label="充装许可证号"></el-table-column>
                <el-table-column show-overflow-tooltip prop="personnelName" label="负责人"></el-table-column>
                <el-table-column show-overflow-tooltip prop="phone" label="负责人联系电话"></el-table-column>
            ...
            <el-dialog title="查看信息" :visible.sync="dialogTableVisible5" class="biaoge1">
              <span slot="title">
                <span style="color: #23cdfd">查看信息</span>
              </span>
              <el-table border :data="gridData9">
                ...
                <el-table-column show-overflow-tooltip prop="idCard" label="身份证"></el-table-column>
                <el-table-column show-overflow-tooltip prop="labelNo" label="标签号"></el-table-column>
                <el-table-column show-overflow-tooltip prop="operatorsNo" label="作业人员证"></el-table-column>
                <el-table-column show-overflow-tooltip prop="noDateStart" label="发证日期">
                  <template slot-scope="socpe">{{socpe.row.unitIssueTime | fmtdate}}</template>
                </el-table-column>
                <el-table-column show-overflow-tooltip prop="noDateEnd" label="换证日期">
                  <template slot-scope="socpe">{{socpe.row.unitChangeTime | fmtdate}}</template>
                </el-table-column>
                <el-table-column show-overflow-tooltip prop="cardInformation" label="发卡信息"></el-table-column>
              </el-table>
              <el-pagination
                style="padding:5px 15px 15px 15px;display:flex; justify-content: flex-end;"
                background
      ...
      <div style="width:100%">
        <div class="big_center_chart_title">车用气瓶加气充装记录</div>
        <div style="overflow-y:hidden;">
          <div class="box8">
            <div class="big_center_chart">
              <!-- 充装单位，车牌号，标签号，气瓶编号，充装时间，充装量，充装员，下次检验时间 -->
              <tr>
                <th>充装单位</th>
                <th>车牌号</th>
                <th>标签号</th>
              ...
              <tr v-for="item in gridData11">
                <td>{{item.unitName}}</td>
                <td>{{item.carNumber}}</td>
                <td>{{item.appId}}</td>
                <td>{{item.gasId}}</td>
                <td>{{item.createTime}}</td>
                <td>{{item.fillingVolume}}</td>
                <td>{{item.userName}}</td>
        ...
        <el-row class="elb">
          ...
          <div ref="tlb" class="box11" :style="active">
            <el-table border :data="gridData4">
              ...
              <el-table-column prop="statscount" label="数量"></el-table-column>
          ...
          <div @mouseover="mouseOver1" @mouseleave="mouseLeave1" class="d1" style="width:100%;">
            <el-button type="primary" round class="elb1">即将超期:{{this.chaoqi}}</el-button>
          </div>
          ...
          <div @mouseover="mouseOver2" @mouseleave="mouseLeave2" style="width:100%;">
...
import img10 from "../../assets/images/buhege.svg";
import img11 from "../../assets/images/hege.svg";
...
export default {
  ...
  async created() {
    ...
    this.change8();
    this.paly();
    const res6 = await this.$http.get(`/car-stats/mapUnit`);
    console.log(res6);
    ...
    this.markerPoint.forEach(e => {
      console.log(e);
      e.isAlive == "0" ? (e.status = img10) : (e.status = img11);
    });
  },
  filters: {
  ...
  mounted() {
    ...
    //   //自定义滚动 出勤率
    //   if (
    //     this.activeIndex < this.gridData11.length /* this.towerListArr.length */
    //   ) {
    //     this.activeIndex += 1;
    //   } else {
    //     this.activeIndex = 0;
  ...
  methods: {
    ...
    async change2() {
      const res9 = await this.$http.get(`/car-stats/checkState/0`);
      this.bhege = res9.data.data;

      const res1 = await this.$http.get(`/car-stats/checkState/1`);
      this.hege = res1.data.data;

      const res2 = await this.$http.get(`/car-stats/checkState/3`);
      this.chaoqi = res2.data.data;
    },
    async change3() {
      const res10 = await this.$http.get(`/car-stats/checkState/2`);
      this.yuqi = res10.data.data;

      //检验状态合格详细信息
      const res11 = await this.$http.get(`/car-stats/findByTypeAndUnit/1`);
      console.log(res11);
      this.gridData4 = res11.data.data;

      // for(var i=0;i<this.gridData4.length;i++){
      //   console.log(this.gridData4[i].pre);
      //   var bfb=this.gridData4[i].pre*100+'%';
      //   this.gridData4=bfb
      // }
    },
    ...
    async change5() {
      //检验状态即将超期详细信息
      const res14 = await this.$http.get(`/car-stats/findByTypeAndUnit/3`);
      console.log(res14);
      this.gridData5 = res14.data.data;

      //检验状态不合格详细信息
      const res15 = await this.$http.get(`/car-stats/findByTypeAndUnit/0`);
      this.gridData6 = res15.data.data;

      //检验状态超期未检详细信息
      const res16 = await this.$http.get(`/car-stats/findByTypeAndUnit/2`);
      this.gridData8 = res16.data.data;
    },
    async change6() {},
    async change7() {
      //已登记气瓶数表单
      const res18 = await this.$http.get(
        `/car-stats/findDoucument?pageSize=${this.pageSize}&currPage=${this.currPage}`
      );
      this.gridData2 = res18.data.data.list;
      this.total2 = res18.data.data.totalCount;

      const res21 = await this.$http.get(`/car-stats/issuCardNum`);
      this.fa = res21.data.data;

      //发证员工表单
      const res22 = await this.$http.get(
        `/car-stats/issuCard?pageSize=${this.pageSize}&currPage=${this.currPage}`
      );
      console.log(res22);
      this.gridData9 = res22.data.data.list;
      this.total5 = res22.data.data.totalCount;

      const res23 = await this.$http.get(`/car-stats/findHardWare`);
      this.gridData3 = res23.data.data;

      //充装站表单
      const res12 = await this.$http.get(
        `/car-stats/fillUint/2?pageSize=${this.pageSize}&currPage=${this.currPage}`
      );
      console.log(res12);
      ...
      //检验站表单
      const res24 = await this.$http.get(
        `/car-stats/fillUint/3?pageSize=${this.pageSize}&currPage=${this.currPage}`
      );
      this.gridData10 = res24.data.data.list;
      this.total4 = res24.data.data.totalCount;

      //使用单位表单
      const res13 = await this.$http.get(
        `/car-stats/fillUint/4?pageSize=${this.pageSize}&currPage=${this.currPage}`
      );
    ...
    async change8() {
      //车用气瓶充装记录表单
      const res25 = await this.$http.get(`/car-stats/findCarRecord`);
      console.log(res25);
      this.gridData11 = res25.data.data.list;
    },
    //百度地图初始化
    handler({ BMap, map }) {},
    // 关闭信息窗口 @close 自带的方法
    ...
    async drawMap2() {
      //归属单位发布
      const res27 = await this.$http.get(`/car-stats/unitCount`);
      console.log(res27);
    ...
    async drawMap3() {
      //本月气站活跃情况
      const res17 = await this.$http.get(`/car-stats/recordCount`);
      const datad1 = [];
      const datadd1 = [];
      const zhexian = res17.data.data;
      for (var key in res17.data.data) {
        datad1.push(res17.data.data[key].unitname);

        datadd1.push(res17.data.data[key].num);
      }
      console.log(datad1);
      const option = {
        color: ["#4e72b8"],
    ...
    async drawMap4() {
      const res28 = await this.$http.get(`/car-stats/countYear`);
      const datad = [];
      const datadd = [];
      const zhexian = res28.data.data;
      for (var key in res28.data.data) {
        datad.push(res28.data.data[key].year);
        datadd.push(res28.data.data[key].num);
      }
    ...
    async getUserlist() {
      const res = await this.$http.get(`/car-stats/findUnitNumber/2`);
      this.cong = res.data.data;

      const res19 = await this.$http.get(`/car-stats/findUnitNumber/3`);
      this.jian = res19.data.data;

      const res20 = await this.$http.get(`/car-stats/findUnitNumber/4`);
      this.shi = res20.data.data;

      const res1 = await this.$http.get(`/car-stats/findUnitNumber/5`);
      this.pei = res1.data.data;

      const res2 = await this.$http.get(`/car-stats/findBottleNumber`);
      this.deng = res2.data.data;

      const res3 = await this.$http.get(`/car-stats/findHardWare`);
      this.jia = res3.data.data;

      // const res4 = await this.$http.get(`/car-stats/findByIsBlack`);
      // this.hei = res4.data.data;

      // const res5 = await this.$http.get(`/car-stats/findByUseUnit`);
      // return res5.data.data[0].buildingUser;
    },
    change() {
      this.gridData11.push(this.gridData11[0]); //把第一条数据插入数组最后一条
      // console.log(this.gridData11.push(this.gridData11[0]));
      // console.log(this.gridData11.shift());
      this.gridData11.shift();
      // this.gridData11.splice(0,1,this.gridData11[0])
    },
    paly() {
