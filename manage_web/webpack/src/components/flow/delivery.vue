<template>
  <el-card class="box-card">
    ...
    <my-bread level1="数据平台" level2="液化气瓶" level3="配送中气瓶"></my-bread>
    <!--搜索  -->
    <el-row>
      <el-col :span="21">
        <el-input
    ...
    <template>
      ...
      >
        ...
        <el-table-column prop="useStatus" :formatter="ifendcase" label="检验状态"></el-table-column>
    ...
    <el-dialog title="添加用户" :visible.sync="dialogFormVisibleAdd">
      <el-form :model="form" :rules="rules">
        ...
        <el-form-item label="配送人员" label-width="100px" prop="operatePerson">
  ...
  methods: {
    ...
    async EditUser() {
      ...
      const res = await this.$http.post(
        `/public/gas/manageGasBasicCylinder/login/update`,
        this.editUserForm
      );
    ...
    async searchUser() {
      const res = await this.$http.get(
        `/public/gas/manageGasBasicCylinder/list?vestingCode=2&vestingUnit=${this.vestingUnit}&useStatus=${this.useStatus}&pageSize=${this.pageSize}&currPage=${this.page}&regId=${this.regId}&gasId=${this.gasId}`
      );
